...
public class MinerItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (!worldIn.isClientSide) {
            playerIn.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 12000, 4));
            playerIn.addEffect(new MobEffectInstance(MobEffects.NIGHT_VISION, 12000, 0));
            playerIn.awardStat(Stats.ITEM_USED.get(this));
            if (!playerIn.getAbilities().instabuild) {
                itemstack.shrink(1);
                ServerLevel world = (ServerLevel) playerIn.getCommandSenderWorld();
                ((ServerLevel) playerIn.getCommandSenderWorld()).sendParticles(ParticleTypes.MYCELIUM, playerIn.getX(), playerIn.getY(), playerIn.getZ(), 5000, 1D, 2D, 1D, 0.01);
            }
