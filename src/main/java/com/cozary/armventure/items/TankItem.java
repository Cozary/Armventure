...
public class TankItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (!worldIn.isClientSide) {
            playerIn.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 17000, 1));
            playerIn.awardStat(Stats.ITEM_USED.get(this));
            if (!playerIn.getAbilities().instabuild) {
                itemstack.shrink(1);
                ServerLevel world = (ServerLevel) playerIn.getCommandSenderWorld();
                ((ServerLevel) playerIn.getCommandSenderWorld()).sendParticles(ParticleTypes.CAMPFIRE_SIGNAL_SMOKE, playerIn.getX(), playerIn.getY(), playerIn.getZ(), 200, 1D, 1D, 1D, 0.1);
                ((ServerLevel) playerIn.getCommandSenderWorld()).sendParticles(ParticleTypes.FLAME, playerIn.getX(), playerIn.getY(), playerIn.getZ(), 200, 1D, 1D, 1D, 0.1);
            }
