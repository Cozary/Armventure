...
public class DefinitiveItem extends Item {

    public DefinitiveItem() {
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (!worldIn.isClientSide) {
            playerIn.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, 20, 500));
            playerIn.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 20, 500));
            playerIn.awardStat(Stats.ITEM_USED.get(this));
            if (!playerIn.getAbilities().instabuild) {
                itemstack.shrink(1);
                ((ServerLevel) playerIn.getCommandSenderWorld()).sendParticles(ParticleTypes.DRAGON_BREATH, playerIn.getX(), playerIn.getY(), playerIn.getZ(), 500, 1D, 1D, 1D, 0.1);
            }
