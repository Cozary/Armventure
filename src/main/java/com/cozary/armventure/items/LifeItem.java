...
public class LifeItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (!worldIn.isClientSide) {
            playerIn.addEffect(new MobEffectInstance(MobEffects.HEALTH_BOOST, 12000, 1));
            playerIn.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 1200, 1));
            playerIn.awardStat(Stats.ITEM_USED.get(this));
            if (!playerIn.getAbilities().instabuild) {
                itemstack.shrink(1);
                ServerLevel world = (ServerLevel) playerIn.getCommandSenderWorld();
                ((ServerLevel) playerIn.getCommandSenderWorld()).sendParticles(ParticleTypes.WITCH, playerIn.getX(), playerIn.getY(), playerIn.getZ(), 500, 1D, 1D, 1D, 0.1);
            }
